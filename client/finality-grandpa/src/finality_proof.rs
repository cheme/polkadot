		...
		BE: Backend<Block> + Send + Sync + 'static,
{
	fn authorities(&self, block: &BlockId<Block>) -> ClientResult<AuthorityList> {
		...
		self.storage(block, &storage_key)?
			.and_then(|encoded| {
				let auths = VersionedAuthorityList::decode(&mut encoded.0.as_slice()).ok();
				auths.as_ref().map(|auths| println!("auths {:?}", &auths.0));
				auths
			})
		...
		B: Backend<Block> + Send + Sync + 'static,
{
	...
	{
		let pp = FinalityProofProvider { backend, authority_provider: Arc::new(authority_provider) };

		let mut hash = Block::Hash::default();
		let mut hash_fin = Block::Hash::default();
		hash.as_mut().copy_from_slice(&[
			172, 53, 24, 88, 141, 84, 39, 152, 59, 154, 176, 18, 77, 204, 57, 159, 80, 48, 234, 65, 206, 202, 77, 235, 40, 242, 62, 192, 135, 158, 167, 72
...
//			161, 76, 240, 39, 225, 32, 158, 75, 128, 167, 55, 169, 229, 253, 91, 243, 42, 227, 244, 55, 166, 159, 236, 180, 9, 19, 231, 81, 94, 200, 69, 216,
		...
		hash_fin.as_mut().copy_from_slice(&[
			69, 108, 94, 2, 142, 87, 244, 222, 64, 75, 182, 70, 7, 8, 184, 159, 25, 147, 179, 150, 121, 165, 183, 59, 156, 182, 9, 174, 174, 132, 25, 240
//			161, 76, 240, 39, 225, 32, 158, 75, 128, 167, 55, 169, 229, 253, 91, 243, 42, 227, 244, 55, 166, 159, 236, 180, 9, 19, 231, 81, 94, 200, 69, 216,
