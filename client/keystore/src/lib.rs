...
///
/// Safe usage fo the store should adhere to trait `BareCryptoStore`.
/// Be aware when manipulating pairs accessed from other api that it could
/// be leaked in memory and shall only be kept for a short duration on
/// the stack.
pub struct Store {
	path: Option<PathBuf>,
...
impl Store {
	...
	/// Places it into the file system store.
	///
	/// Returns the pair with unencrypted key.
	/// The pair shall be drop from stack as soon as possible
	/// to avoid leaking secret in memory.
	...
	pub fn insert_ephemeral_from_seed_by_type<Pair: PairT>(
		...
		// TODO  ? let pair = Pair::from_string(seed, self.password()).map_err(|_| Error::InvalidSeed)?;
