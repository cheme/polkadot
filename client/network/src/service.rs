...
impl<B: BlockT + 'static, H: ExHashT, P: StorageProof> NetworkWorker<B, H, P> {
	...
	pub fn new(params: Params<B, H, P>) -> Result<NetworkWorker<B, H, P>, Error> {
		...
		// Build the swarm.
		let (mut swarm, bandwidth): (Swarm<B, H, P>, _) = {
		...
		for addr in &params.network_config.listen_addresses {
			if let Err(err) = Swarm::<B, H, P>::listen_on(&mut swarm, addr.clone()) {
		...
		for addr in &params.network_config.public_addresses {
			Swarm::<B, H, P>::add_external_address(&mut swarm, addr.clone());
		}
	...
	pub fn local_peer_id(&self) -> &PeerId {
		Swarm::<B, H, P>::local_peer_id(&self.network_service)
	}
	...
	pub fn listen_addresses(&self) -> impl Iterator<Item = &Multiaddr> {
		Swarm::<B, H, P>::listeners(&self.network_service)
	}
	...
	pub fn network_state(&mut self) -> NetworkState {
		...
		NetworkState {
			peer_id: Swarm::<B, H, P>::local_peer_id(&swarm).to_base58(),
			listened_addresses: Swarm::<B, H, P>::listeners(&swarm).cloned().collect(),
			external_addresses: Swarm::<B, H, P>::external_addresses(&swarm).cloned().collect(),
...
pub struct NetworkWorker<B: BlockT + 'static, H: ExHashT, P: StorageProof> {
...
impl<B: BlockT + 'static, H: ExHashT, P: StorageProof> Future for NetworkWorker<B, H, P> {
	...
	fn poll(mut self: Pin<&mut Self>, cx: &mut std::task::Context) -> Poll<Self::Output> {
		...
		{
			let external_addresses = Swarm::<B, H, P>::external_addresses(&this.network_service).cloned().collect();
...
impl<B: BlockT + 'static, H: ExHashT, P: StorageProof> Unpin for NetworkWorker<B, H, P> {
}
...
type Swarm<B, H, P> = libp2p::swarm::Swarm<Behaviour<B, H, P>>;

// Implementation of `import_queue::Link` trait using the available local variables.
struct NetworkLink<'a, B: BlockT, H: ExHashT, P: StorageProof> {
	protocol: &'a mut Swarm<B, H, P>,
}

impl<'a, B: BlockT, H: ExHashT, P: StorageProof> Link<B> for NetworkLink<'a, B, H, P> {
	fn blocks_processed(
		&mut self,
		imported: usize,
		count: usize,
