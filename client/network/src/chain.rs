...
pub trait Client<Block: BlockT>: HeaderBackend<Block> + ProofProvider<Block, StorageProof<HashFor<Block>>> + BlockIdTo<Block, Error = Error>
	+ BlockBackend<Block> + HeaderMetadata<Block, Error = Error> + Send + Sync
{}

impl<Block: BlockT, T> Client<Block> for T
	where
		T: HeaderBackend<Block> + ProofProvider<Block, StorageProof<HashFor<Block>>> + BlockIdTo<Block, Error = Error>
			+ BlockBackend<Block> + HeaderMetadata<Block, Error = Error> + Send + Sync
{}
