				...
				Some(GenericProtoOut::CustomProtocolOpen { peer_id, .. }) => {
					for n in 0 .. NUM_PACKETS {
						service1.send_packet(
							&peer_id,
							Message::<Block, sp_state_machine::TrieNodesStorageProof>::BlockResponse(BlockResponse {
								id: n as _,
								blocks: Vec::new(),
							}).encode()
						);
		...
		loop {
			match ready!(service2.poll_next_unpin(cx)) {
				...
				Some(GenericProtoOut::LegacyMessage { message, .. }) => {
					match Message::<Block, sp_state_machine::TrieNodesStorageProof>::decode(&mut &message[..]).unwrap() {
						Message::<Block, sp_state_machine::TrieNodesStorageProof>::BlockResponse(BlockResponse { id: _, blocks }) => {
...
fn basic_two_nodes_requests_in_parallel() {
	...
	let mut to_send = {
		...
		for _ in 0..200 { // Note: don't make that number too high or the CPU usage will explode.
			...
			to_send.push(Message::<Block, sp_state_machine::TrieNodesStorageProof>::BlockResponse(
