...
impl<S: StateBackend<HashFor<B>>, B: BlockT> StateBackend<HashFor<B>> for SyncingCachingState<S, B> {
	...
	fn child_storage_hash(
		...
		self.caching_state().child_storage_hash(child_info, key)
	}
...
mod tests {
	...
	fn smoke() {
		...
		let mut s = CachingState::new(
			InMemoryBackend::<Layout<BlakeTwo256>>::default(),
			shared.clone(),
			Some(root_parent),
		);
		s.cache.sync_cache(
	...
	fn should_track_used_size_correctly() {
		...
		let mut s = CachingState::new(
			InMemoryBackend::<Layout<BlakeTwo256>>::default(), shared.clone(), Some(root_parent.clone()),
		);
...
mod qc {
	...
	impl Mutator {
		...
		fn head_state(&self, hash: H256) -> CachingState<InMemoryBackend<Layout<BlakeTwo256>>, Block> {
			CachingState::new(
				InMemoryBackend::<Layout<BlakeTwo256>>::default(),
				self.shared.clone(),
				Some(hash),
		...
		fn canon_head_state(&self) -> CachingState<InMemoryBackend<Layout<BlakeTwo256>>, Block> {
		...
		fn mutate_static(
			&mut self,
			action: Action,
		) -> CachingState<InMemoryBackend<Layout<BlakeTwo256>>, Block> {
		...
		fn mutate(
			&mut self,
			action: Action,
		) -> Result<CachingState<InMemoryBackend<Layout<BlakeTwo256>>, Block>, ()> {
