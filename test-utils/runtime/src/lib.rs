...
use sp_trie::{PrefixedMemoryDB, BackendProof, SimpleProof as StorageProof};
...
fn test_witness(proof: StorageProof, root: crate::Hash) {
	...
	let db: sp_trie::MemoryDB<crate::Hashing> = proof.into_partial_db().unwrap();
	let backend = sp_state_machine::TrieBackend::<_, crate::Hashing, StorageProof>::new(
		db,
		root,
	);
...
mod tests {
	...
	use sp_trie::SimpleProof as StorageProof;
