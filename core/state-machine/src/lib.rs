...
pub struct StateMachine<'a, B, H, N, T, Exec, C> where H: Hasher<Out=H256>, B: Backend<H> {
	backend: &'a B,
	exec: &'a Exec,
	method: &'a str,
	call_data: &'a [u8],
...
impl<'a, B, H, N, T, Exec, C> StateMachine<'a, B, H, N, T, Exec, C> where
	H: Hasher<Out=H256>,
	Exec: CodeExecutor,
	B: Backend<H>,
	T: ChangesTrieStorage<H, N>,
	C: client::Externalities,
	...
	C: client::Externalities,
{
	...
	prove_execution_on_trie_backend(trie_backend, overlay, exec, method, call_data, keystore, client)
}
	...
	C: client::Externalities,
{
	...
	let mut sm = StateMachine::<_, H, _, InMemoryChangesTrieStorage<H, u64>, Exec, C>::new(
		&proving_backend, None, None, overlay, exec, method, call_data, keystore, client,
	);
...
{
	let mut sm = StateMachine::<_, H, _, InMemoryChangesTrieStorage<H, u64>, Exec, C>::new(
		trie_backend, None, None, overlay, exec, method, call_data, keystore, client,
	);
...
mod tests {
	...
	fn prove_execution_and_proof_check_works() {
		...
		// check proof locally
		let local_result = execution_proof_check::<Blake2Hasher, _, _>(
			remote_root,
			remote_proof,
			&mut Default::default(),
			&executor,
