...
pub use crate::offstate_backend::{OffstateStorage, OffstateBackendStorage, TODO};
...
/// Note: changes to code will be in place if this call is made again. For running partial
/// blocks (e.g. a transaction at a time), ensure a different method is used.
pub fn prove_execution_on_trie_backend<S, O, H, Exec>(
	trie_backend: &TrieBackend<S, O, H>,
	overlay: &mut OverlayedChanges,
	exec: &Exec,
	method: &str,
	call_data: &[u8],
...
pub fn execution_proof_check_on_trie_backend<H, Exec>(
	trie_backend: &TrieBackend<MemoryDB<H>, TODO, H>,
	overlay: &mut OverlayedChanges,
	exec: &Exec,
	method: &str,
	call_data: &[u8],
...
pub fn prove_read_on_trie_backend<S, O, H>(
	trie_backend: &TrieBackend<S, O, H>,
	key: &[u8]
) -> Result<(Option<Vec<u8>>, Vec<Vec<u8>>), Box<dyn Error>>
where
	...
	H::Out: Ord,
{
	let proving_backend = proving_backend::ProvingBackend::<_, _, H>::new(trie_backend);
	let result = proving_backend.storage(key).map_err(|e| Box::new(e) as Box<dyn Error>)?;
	Ok((result, proving_backend.extract_proof()))
}

/// Generate storage read proof on pre-created trie backend.
pub fn prove_child_read_on_trie_backend<S, O, H>(
	trie_backend: &TrieBackend<S, O, H>,
	storage_key: &[u8],
	key: &[u8]
) -> Result<(Option<Vec<u8>>, Vec<Vec<u8>>), Box<dyn Error>>
...
pub fn read_proof_check_on_proving_backend<H>(
	proving_backend: &TrieBackend<MemoryDB<H>, TODO, H>,
	key: &[u8],
) -> Result<Option<Vec<u8>>, Box<dyn Error>>
where
	H: Hasher,
