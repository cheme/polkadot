...
use std::{fmt, panic::UnwindSafe, result, marker::PhantomData, collections::BTreeMap};
use log::warn;
use hash_db::Hasher;
use heapsize::HeapSizeOf;
use parity_codec::{Decode, Encode};
use primitives::{storage::well_known_keys, NativeOrEncoded, NeverNativeValue, KeySpace};
...
pub fn execution_proof_check_on_trie_backend<H, Exec>(
	trie_backend: &TrieBackend<BTreeMap<KeySpace, MemoryDB<H>>, H>,
	overlay: &mut OverlayedChanges,
	exec: &Exec,
	method: &str,
	call_data: &[u8],
...
pub fn read_proof_check_on_proving_backend<H>(
	proving_backend: &TrieBackend<BTreeMap<KeySpace, MemoryDB<H>>, H>,
	key: &[u8],
) -> Result<Option<Vec<u8>>, Box<Error>>
where
	H: Hasher,
