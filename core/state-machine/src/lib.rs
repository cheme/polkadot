...
use primitives::{storage::well_known_keys, NativeOrEncoded, NeverNativeValue, SubTrie};
...
pub trait Externalities<H: Hasher> {
  ...
  /// TODO change if : no need for storage key (defined in SubTrie)
	fn set_child_trie(&mut self, storage_key: &[u8], subtrie: &SubTrie) {
		let mut key_for = well_known_keys::CHILD_STORAGE_KEY_PREFIX.to_vec();
		Encode::encode_to(&storage_key, &mut key_for);
	...
	fn set_child_storage(&mut self, subtrie: &SubTrie, key: Vec<u8>, value: Vec<u8>) -> bool {
	...
	fn clear_child_storage(&mut self, subtrie: &SubTrie, key: &[u8]) -> bool {
	...
	/// TODO EMCH audit if not easier to not use SubTrie here
...
mod tests {
    ...
    ext.set_child_trie(&b"testchild"[..], &SubTrie::new(b"testchild_keyspace".to_vec(), b"testchild".to_vec()));
    let subtrie = ext.get_child_trie(&b"testchild"[..]).expect("set above");
		assert!(ext.set_child_storage(&subtrie, b"abc".to_vec(), b"def".to_vec()));
