...
pub fn new<'a, H, N, B, T, O, Exec, C, BB>(
	client: &'a C,
  build_backend: &'a mut BB,
	...
	method: &'a str,
	call_data: &'a [u8],
) -> StateMachine<'a, H, N, B, T, O, Exec, C, BB> {
	StateMachine {
		client,
    build_backend,
		backend,
...
pub struct StateMachine<'a, H, N, B, T, O, Exec, C, BB> {
	client: &'a C,
  build_backend: &'a mut BB,
/// TODO EMCH client and trie_backend seems redundant (can get backend on block)
	backend: &'a B,
	changes_trie_storage: Option<&'a T>,
	offchain_ext: Option<&'a mut O>,
	overlay: &'a mut OverlayedChanges,
...
impl<'a, H, N, B, T, O, Exec, C, BB> StateMachine<'a, H, N, B, T, O, Exec, C, BB> where
	H: Hasher,
	Exec: CodeExecutor<H>,
	B: Backend<H>,
  BB: 'a + FnMut(H::Out) -> B,
...
where
	...
	H::Out: Ord + 'static,
{
  let trie_backend = Rc::new(trie_backend);
  let tb_proto = trie_backend.clone();
	let proving_backend = proving_backend::ProvingBackend::new(tb_proto.as_ref());
  let mut build_backend = move |hash| {
    // TODO EMCH this require some abstraction over proving backend eg AsRef.
    let trie_backend = TrieBackend::new(tb_proto.clone().into_storage(), hash);

    proving_backend::ProvingBackend::new(&trie_backend)
  };
	let mut sm = StateMachine {
		client,
    build_backend: &mut build_backend,
...
where
	...
	H::Out: Ord + 'static,
{
  let proof_proto = proof.clone();
	let trie_backend = create_proof_check_backend::<H>(root.into(), proof)?;
  let bb = move |hash: H::Out| {
    create_proof_check_backend::<H>(hash.into(), proof_proto.clone())
      .expect("TODO EMCH return result from backend builder")
  };
	execution_proof_check_on_trie_backend(client, &mut bb, &trie_backend, overlay, exec, method, call_data)
}

/// Check execution proof on proving backend, generated by `prove_execution` call.
pub fn execution_proof_check_on_trie_backend<H, Exec, C, BB>(
	client: &C,
	build_backend: &mut BB,
...
where
	...
	Exec: CodeExecutor<H>,
	C: crate::client::Externalities<H>,
  BB: FnMut(H::Out) -> TrieBackend<MemoryDB<H>, H>,
	H::Out: Ord + 'static,
{
	let mut sm = StateMachine {
		client,
