...
mod tests {
	...
	fn code_is_executed_locally_or_remotely() {
		...
		backend.blockchain().insert(hash0, header0, None, None, NewBlockState::Final, 0).unwrap();
		backend.blockchain().insert(hash1, header1, None, None, NewBlockState::Final, 0).unwrap();
