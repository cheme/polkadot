...
impl sp_state_machine::Storage<Blake2Hasher> for Storage {
	fn get(&self, key: &Hash, prefix: Prefix) -> Result<Option<Vec<u8>>, String> {
		let key = sp_trie::prefixed_key::<Blake2Hasher>(key, prefix);
...
impl core::Benchmark for TrieReadBenchmark {
	fn run(&mut self, mode: Mode) -> std::time::Duration {
		...
		let storage: Arc<dyn sp_state_machine::Storage<Blake2Hasher>> =
