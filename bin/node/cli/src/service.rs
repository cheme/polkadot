...
pub fn new_full_params(config: Configuration) -> Result<(
	...
	let provider = Arc::new(FinalityProofClient(client.clone())) as Arc<dyn StorageAndProofProvider<_, _>>;
...
pub fn new_light_base(config: Configuration) -> Result<(
	...
	let (client, backend, keystore, task_manager, on_demand) = sc_service::new_light_parts::<
		Block,
		RuntimeApi,
		Executor,
