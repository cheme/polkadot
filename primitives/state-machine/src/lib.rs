...
mod tests {
	...
	fn prove_execution_and_proof_check_works() {
		...
		// check proof locally
		let local_result = execution_proof_check::<ProvingBackend, BlakeTwo256, u64, _>(
			remote_root,
			remote_proof,
			&mut Default::default(),
			&executor,
	...
	fn prove_read_and_proof_check_works() {
		...
		let remote_proof = prove_read(remote_backend, &[b"value2"]).unwrap();
 		// check proof locally
		let local_result1 = read_proof_check::<ProvingBackend, BlakeTwo256, _>(
			remote_root,
			remote_proof.clone(),
			&[b"value2"],
		).unwrap();
		let local_result2 = read_proof_check::<ProvingBackend, BlakeTwo256, _>(
			remote_root,
			remote_proof.clone(),
			&[&[0xff]],
		).is_ok();
		...
		let local_result1 = read_child_proof_check::<ProvingBackend, BlakeTwo256, _>(
			remote_root,
			remote_proof.clone(),
			child_info,
			&[b"value3"],
		).unwrap();
		let local_result2 = read_child_proof_check::<ProvingBackend, BlakeTwo256, _>(
			remote_root,
			remote_proof.clone(),
			child_info,
			&[b"value2"],
