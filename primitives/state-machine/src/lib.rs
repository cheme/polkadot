	...
	N: crate::changes_trie::BlockNumber,
{
	...
	pub fn new(
		...
		Self {
			...
			proving_mode: Default::default(),
		}
	}

	pub(crate) fn set_proving_mode(
		&mut self,
		mode: ProvingMode,
	) {
		if let ProvingMode::WithoutTransaction = mode {
			self.overlay.use_without_transaction_proof();
		}
		self.proving_mode = mode;
	}
...
where
	...
	N: crate::changes_trie::BlockNumber,
{
	prove_execution_on_trie_backend_with_mode::<S, H, N, Exec>(
		trie_backend,
		overlay,
		exec,
		spawn_handle,
...
/// Note: changes to code will be in place if this call is made again. For running partial
/// blocks (e.g. a transaction at a time), ensure a different method is used.
pub fn prove_execution_on_trie_backend_with_mode<S, H, N, Exec>(
	trie_backend: &TrieBackend<S, H>,
	overlay: &mut OverlayedChanges,
	exec: &Exec,
	spawn_handle: Box<dyn CloneableSpawn>,
	...
	N: crate::changes_trie::BlockNumber,
{
	...
	let result = {
		...
		sm.set_proving_mode(proving_mode);
...
where
	...
	N: crate::changes_trie::BlockNumber,
{
	execution_proof_check_on_trie_backend_with_mode::<H, N, Exec>(
		trie_backend,
		overlay,
		exec,
		spawn_handle,
...
/// Check execution proof on proving backend, generated by `prove_execution` call.
pub fn execution_proof_check_on_trie_backend_with_mode<H, N, Exec>(
	trie_backend: &TrieBackend<MemoryDB<H>, H>,
	overlay: &mut OverlayedChanges,
	exec: &Exec,
	spawn_handle: Box<dyn CloneableSpawn>,
...
/// Depending on usage, we may need to produce transaction payload or not.
/// Additional queries are needed to produce transaction payload.
pub enum ProvingMode {
	/// This is also the mode to use for standard execution and therefore the default.
	///
	/// For producing a proof, thhis should be use instead of `WithoutTransaction`
	/// when we need knowledge of the deletion, eg use of `changes_root` host function.
	/// We use the proving backend just as a straight proxy to its inner backend.
	/// Proof content can be bigger than `WithoutTransaction`.
	Standard,
	/// We do not need state updates, this is the default case for proof.
	/// This could be use almost everywhere, except if we need handle of deletion,
	/// for instance a call to the host function `changes_root`.
	/// For technical reason, this does not support revert of modification and is
	/// limited in scope to successfull extrinsic processing. In theory the filter
	/// overlay from previously successfull processing could be used again or drop
	/// on failure.
	WithoutTransaction,
	/// We need top produce both a proof and provide state updates.
	/// Proof content is the same as `WithoutTransaction`, so it is also
	/// limited to call that do not need to register deletion.
	/// This support reverting extrinsic.
	WithTransaction,
}
