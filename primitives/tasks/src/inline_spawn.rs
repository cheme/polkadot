...
>(
	...
	#[cfg(feature = "std")]
	{
		crate::common::process_task_inner::<HostLocal, _>(task, async_ext, handle, instance_ref)
	}
	#[cfg(not(feature = "std"))]
	{
		crate::common::process_task_inner::<HostLocal>(task, async_ext, handle)
	}
}
		...
		fn join(&self, handle: TaskId, calling_ext: &mut dyn Externalities) -> Option<Vec<u8>> {
			...
			let worker_result = match self.0.borrow_mut().tasks.remove(&handle) {
				Some(task) => {
					...
					#[cfg(not(feature = "std"))]
					process_task_inline_inner::<HostLocal>(task.task, task.ext, nested, handle)
				},
