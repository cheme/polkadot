...
pub fn read_trie_value<L: TrieConfiguration, DB: hash_db::HashDBRef<L::Hash, trie_db::DBValue, L::Meta>>(
	...
	Ok(TrieDB::new_with_layout(&*db, root, layout)?.get(key).map(|x| x.map(|val| val.to_vec()))?)
}
...
>(
	...
	Ok(TrieDB::new_with_layout(&*db, root, layout)?.get_with(key, query).map(|x| x.map(|val| val.to_vec()))?)
}
		...
		DB: hash_db::HashDBRef<L::Hash, trie_db::DBValue, L::Meta>
{
	...
	let trie = TrieDB::new_with_layout(&db, &root, layout)?;
	let iter = trie.iter()?;
	...
	T: From<&'static [u8]>,
{
	...
	fn get_with_meta(&self, key: &H::Out, prefix: Prefix) -> Option<(T, M)> {
		let derived_prefix = keyspace_as_prefix_alloc(self.1, prefix);
		self.0.get_with_meta(key, (&derived_prefix.0, derived_prefix.1))
	}
...
mod tests {
	...
	fn default_trie_root() {
		...
		let mut empty = TrieDBMut::<Layout>::new_with_layout(&mut db, &mut root, layout.clone());
		empty.commit();
